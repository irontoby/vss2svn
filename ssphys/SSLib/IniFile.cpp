...
bool CIniFile::Load(string FileName, vector<IniRecord>& content)
{
	...
	while(!std::getline(inFile, s).eof())									// Read until the end of the file
	{
		...
		{
			IniRecord r;														// Define a new record
...
bool CIniFile::Save(string FileName, vector<IniRecord>& content)
{
...
string CIniFile::Content(string FileName)
{
	...
	vector<IniRecord> content;													// Holds the current record													// Holds the current record
...
vector<CIniFile::IniRecord> CIniFile::GetSection(string SectionName, string FileName)
{
	vector<IniRecord> data;													// Holds the return data
	vector<IniRecord> content;													// Holds the current record													// Holds the current record
...
bool CIniFile::RecordExists(string KeyName, string SectionName, string FileName)
{
	...
	{
    UCase (KeyName, true);
		vector<IniRecord>::iterator iter = std::find_if(content.begin(), 
				content.end(), 
...
vector<CIniFile::IniRecord> CIniFile::GetRecord(string KeyName, string SectionName, string FileName)
{
	vector<IniRecord> data;													// Holds the return data
	vector<IniRecord> content;													// Holds the current record													// Holds the current record
    ...
    UCase (KeyName, true);
		vector<IniRecord>::iterator iter = std::find_if(content.begin(), 
				content.end(), 
				CIniFile::RecordSectionKeyIs(SectionName,KeyName));			// Locate the IniRecord

		if (iter == content.end()) return data;								// The IniRecord was not found

		data.push_back (*iter);												// The IniRecord was found
	}
	return data;															// Return the IniRecord
}

string CIniFile::GetValue(string KeyName, string SectionName, string FileName)
{
	vector<IniRecord> content = GetRecord(KeyName,SectionName, FileName);		// Get the IniRecord
...
bool CIniFile::SetValue(string KeyName, string Value, string SectionName, string FileName)
{
    ...
    UCase (KeyName, true);
		...
		{
      IniRecord s ("",' ',SectionName,"","");							// Define a new section
			IniRecord r ("",' ',SectionName,KeyName,Value);					// Define a new record
...
bool CIniFile::RenameSection(string OldSectionName, string NewSectionName, string FileName)
{
	...
	if (Load(FileName, content))											// Make sure the file is loaded
	{
		for(vector<IniRecord>::iterator iter = content.begin(); 
...
bool CIniFile::CommentSection(char CommentChar, string SectionName, string FileName)
{
	...
	if (Load(FileName, content))											// Make sure the file is loaded
	{
		for(vector<IniRecord>::iterator iter = content.begin(); iter < content.end(); iter++)
		{
...
bool CIniFile::DeleteRecord(string KeyName, string SectionName, string FileName)
{
    ...
    UCase (KeyName, true);
		...
		content.erase(iter);												// Remove the IniRecord
		return Save(FileName,content);										// Save
...
bool CIniFile::SetSectionComments(string Comments, string SectionName, string FileName)
{
	...
	if (Load(FileName, content))											// Make sure the file is loaded
	{
		for(vector<IniRecord>::iterator iter = content.begin(); iter < content.end(); iter++)									// Loop through the records
		{
...
vector<CIniFile::IniRecord> CIniFile::GetSections(string FileName)
{
	vector<IniRecord> data;													// Holds the return data
	vector<IniRecord> content;													// Holds the current record													// Holds the current record
...
{
	vector<CIniFile::IniRecord> content;										// Used to hold the sorted content
	vector<CIniFile::IniRecord> sections = GetSections(FileName);				// Get a list of Sections
	...
	if(!sections.empty())													// Is there anything to process?
	{
		...
		for(vector<IniRecord>::iterator iter = sections.begin(); iter < sections.end(); iter++) // For each Section
		{																		
			content.push_back(*iter);										// Add the sorted Section to the content

			vector<CIniFile::IniRecord> records = GetSection(iter->Section ,FileName); // Get a list of Records for this section
			...
			for(vector<IniRecord>::iterator it = records.begin(); it < records.end(); it++) // For each IniRecord
				content.push_back(*it);										// Add the sorted IniRecord to the content
		}
...
{
	vector<IniRecord> content;													// Create empty content
	return Save(FileName,content);											// Save
}
