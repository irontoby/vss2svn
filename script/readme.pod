...
This means that if you convert a VSS repository "into" an SVN repository that already has commits, or if you convert your VSS repository(ies) in multiple stages, you could end up with out-of-order svn:date values. This could render such SVN features as blame, checkout by date, etc. unusable. 
